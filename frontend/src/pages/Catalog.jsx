...
const Catalog = () => {
    ...
    useEffect(() => {
        const fetchCategoryId = async () => {
            ...
            categories.length && setCategoryId(
                categories.find((cat) => cat.name.trim().toLowerCase() === ctName.trim().toLowerCase())._id
            )
            setLoading(false)
        }
        fetchCategoryId()
    ...
    return catelogPageData && (
        <>
            <div className='bg-richblack-800 text-richblack-5'>
                <div className='w-11/12 mx-auto flex flex-row gap-10 py-10 justify-between'>
                    <div className='flex flex-col gap-2 w-[70%]'>
                        <p className='text-sm text-richblack-300'>Home / Catalog / <span className='text-yellow-50'>{ctName}</span> </p>
                        <h1 className='text-3xl font-medium'>{ctName}</h1>
                        <p className='text-sm text-richblack-200'>{catelogPageData.selectedCategory.description}</p>
                    </div>
            ...
            <div className='bg-richblack-900 text-richblack-5 mb-10'>
                <div className='w-11/12 mx-auto py-10 flex flex-col gap-28'>
                    <div className='flex flex-col gap-4'>
                        ...
                        <div className='flex flex-row gap-5 border-b border-richblack-400'>
                            <div onClick={() => setCurrTab('Most popular')} className={`px-2 py-1 cursor-pointer ${currTab === 'Most popular' ? "border-b-[1.5px] border-yellow-50 text-yellow-50" : ""}`}>Most popular</div>

                            <div onClick={() => setCurrTab('New')} className={`px-2 py-1 cursor-pointer ${currTab === 'New' ? "border-b-[1.5px] border-yellow-50 text-yellow-50" : ""}`}>New</div>
                        </div>
                    ...
                    <div className='flex flex-col gap-4'>
                        <h1 className='text-3xl font-semibold'>Top courses in <span>{catelogPageData.differentCategory.name}</span></h1>
                        <div>
